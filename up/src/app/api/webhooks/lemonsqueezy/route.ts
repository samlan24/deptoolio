...
async function handleSubscriptionCreated(subscription: any) {
  ...
  await supabase
    ...
    .upsert({
      ...
      plan_type: 'pro',
      period_start: new Date(subscription.attributes.current_period_start).toISOString().split('T')[0], // Convert to date format
      period_end: new Date(subscription.attributes.current_period_end).toISOString().split('T')[0], // Convert to date format
