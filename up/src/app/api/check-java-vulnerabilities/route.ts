...
interface JavaDependencies {
  [packageName: string]: string; // "groupId:artifactId": "version"
}
...
// Helper function to parse Java package names
function parseJavaPackageName(packageName: string): { groupId: string; artifactId: string } | null {
...
// Helper function to normalize Java version
function normalizeJavaVersion(version: string): string {
  // Remove any non-version characters and get the base version
...
function determineSeverity(vuln: OSVVulnerability): string {
  ...
  // Default to high for Java since security is critical in enterprise environments
  return 'high';
}
...
// Query OSV database for a specific Java package
async function queryOSVForJavaPackage(groupId: string, artifactId: string, version?: string): Promise<OSVResponse> {
  // Maven packages in OSV are identified as "groupId:artifactId"
  ...
  if (version) {
    payload.version = normalizeJavaVersion(version);
  }
  ...
  try {
    const response = await fetch("https://api.osv.dev/v1/query", {
      ...
      headers: {
        ...
        "User-Agent": "Java-Package-Vulnerability-Scanner/1.0"
      },
...
export async function POST(request: NextRequest) {
  try {
    const { dependencies } = await request.json() as { dependencies: JavaDependencies };

    if (!dependencies || typeof dependencies !== "object") {
      return NextResponse.json({
        error: "Dependencies required. Expected format: { 'groupId:artifactId': 'version' }"
      }, { status: 400 });
    }
    ...
    const concurrency = 5; // Be respectful to OSV API
    ...
    for (let i = 0; i < packageEntries.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(async ([packageName, version]) => {
          try {
            const parsedPackage = parseJavaPackageName(packageName);
            if (!parsedPackage) {
              console.error(`Invalid package name format: ${packageName}. Expected 'groupId:artifactId'`);
              return;
            }

            const { groupId, artifactId } = parsedPackage;
            const osvResponse = await queryOSVForJavaPackage(groupId, artifactId, version);
    ...
    console.error("Error in Java package vulnerability scan:", error);
    return NextResponse.json({
...
export async function GET() {
  return NextResponse.json({
    message: "Java Package Vulnerability Scanner API",
    usage: "POST request with { dependencies: { 'groupId:artifactId': 'version' } }",
    example: {
      dependencies: {
        "org.apache.commons:commons-lang3": "3.9",
        "com.fasterxml.jackson.core:jackson-databind": "2.12.0",
        "org.springframework:spring-core": "5.2.0"
      }
    },
    response_format: {
      advisories: {
