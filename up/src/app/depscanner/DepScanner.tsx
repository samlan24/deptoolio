...
export default function DepScanner() {
  ...
  const loadFolders = async (repo: Repo) => {
    ...
    try {
      ...
      // Always include Root as the first option with special value
      const foldersWithRoot = [
        { name: "Root", path: "." }, // Use "." instead of ""
        ...(data || []),
      ];
      ...
      setError(
        "Failed to load subfolders, but you can still scan the root directory"
      );
    } finally {
